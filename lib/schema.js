...
function _enumCompositions(schema, path, isArray, cb) {
    if (!cb(path, schema, isArray))
        return;
    Object.keys(schema.properties).forEach(function (name) {
        var prop = schema.properties[name];
        if (prop.type === "object") {
            let cp = path ? path + '.' + name : name;
            _enumCompositions(prop, cp, false, cb);
        }
        else if (prop.type === "array" && prop.items.type === 'object') {
            let cp = path ? path + '.' + name : name;
            _enumCompositions(prop.items, cp, true, cb);
        }
    });
}
function indexesOfSchema(schema, addTextIndex) {
    ...
    // add Indexes
    _enumCompositions(schema, '', false, function (prefix, cs, array) {
        if (cs.indexes) {
